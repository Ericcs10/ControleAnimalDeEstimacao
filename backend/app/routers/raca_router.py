...
from app.schemas.raca_schema import RacaSchema, RacaDBSchema
...
@router.post("/", status_code=status.HTTP_201_CREATED, response_model=RacaDBSchema)
...
@router.get("/", response_model=List[RacaDBSchema])
async def listar_racas(service: RacaService = Depends(get_service)):
    return await service.listar()


@router.get("/{raca_id}", response_model=RacaDBSchema)
...
@router.put("/{raca_id}", response_model=RacaDBSchema)
